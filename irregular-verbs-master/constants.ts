...
import { Verb, Section } from './types';

export const RAW_VERBS_DATA = `abide,abode/abided,abode / abided,"lưu trú, lưu lại "
...
remake,remade,remade,làm lại; chế tạo lai
...
shear /fıə(r); fir /,sheared,shorn/fo:n/ or/ fo:rn/,xén lông (Cừu)
...
strew,strewed,strewn/strewed,"rắc, rãi "
...
export const parseAllVerbs = (): Verb[] => {
  return RAW_VERBS_DATA.split('\n')
...
export const createSections = (verbs: Verb[], perSection: number = 20): Section[] => {
  const sections: Section[] = [];
  for (let i = 0; i < verbs.length; i += perSection) {
    const chunk = verbs.slice(i, i + perSection);
    const id = Math.floor(i / perSection) + 1;
    sections.push({
      id,
      name: `Bài học ${id} (${chunk[0].v1} - ${chunk[chunk.length-1].v1})`,
      verbs: chunk
    });
  }
  return sections;
